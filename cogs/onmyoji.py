...
class OnmyojiArena:
	...
	async def builds(self, ctx, text:str):
		...
		if (heroPID <= 0):
			...
			return
		theway = os.path.join("cogs", "test.db")
		...
		for i in range(4):
			thebuild = os.path.join("data", "ml", heroP, "author"+str(i)+".png")
			...
			worldRank = str(rows[i][1]).strip()
			winRate = str(rows[i][2]).strip()
			matchesPlayed = str(rows[i][3]).strip()
			embed = discord.Embed(title=authorName, colour=discord.Colour(0x05EDFF), url="https://www.mobilelegends.com/hero/detail-" + str(heroPID), description=worldRank)
			embed.set_image(url=builds[i])
			embed.set_author(name=heroP.capitalize() + " Builds", url="https://www.mobilelegends.com/hero/detail-" + str(heroPID))
